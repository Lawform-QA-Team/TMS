/**
 * CLM 법무 검토 승인 / 반려 - Playwright E2E
 *
 * 커버하는 시나리오:
 * 1. CLM 검토 목록 → 첫 번째 항목 진입 (또는 CLM_ID 직접 진입)
 * 2. APPROVE_TYPE=approve → 승인 버튼 클릭 → 확인
...
export async function run(page) {
    ...
    const denyReason = process.env.DENY_REASON ?? '검토 후 반려 처리';
    ...
    if (clmId) {
        ...
        await page.waitForLoadState('networkidle');
        await page.screenshot({ path: `screenshots/${ts()}_clm_legal_list.png` });

        const firstRow = page.locator('table tbody tr').first();
        if (!await firstRow.isVisible()) {
            console.log('[clm_lagel] 검토 목록에 항목 없음 — 스킵');
            return;
        }
        await firstRow.click();
        await page.waitForLoadState('networkidle');
    }
    await page.screenshot({ path: `screenshots/${ts()}_clm_legal_detail.png` });
    ...
    if (approveType === 'deny') {
        ...
        await page.screenshot({ path: `screenshots/${ts()}_clm_legal_deny_modal.png` });
        ...
        await page.waitForTimeout(500);
        await page.screenshot({ path: `screenshots/${ts()}_clm_legal_approve_modal.png` });
    ...
    await page.waitForLoadState('networkidle');
    await page.screenshot({ path: `screenshots/${ts()}_clm_legal_done.png` });
}
