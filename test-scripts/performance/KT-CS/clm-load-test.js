...
// top-level이라 VU 초기화마다(9,500번) 실행되므로, 예전에 .slice()를 썼을 때
// 9,500 VU × 최대 7,600개 파싱이 겹쳐 "cannot allocate memory"로 크래시가 났었다.
...
export function readFlow() {
  const idx = exec.scenario.iterationInTest % READ_POOL_SIZE;
...
export function writeFlow() {
  const idx = WRITE_POOL_OFFSET + (exec.scenario.iterationInTest % WRITE_POOL_SIZE);
  const account = accounts[idx];
  let ok = true;
  let token = null;
  let clmId = null;
