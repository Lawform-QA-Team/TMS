...
### 🧑‍💼 **executive (임원)**
- **설명**: 주요 데이터와 관리성 화면 조회는 가능하지만, 생성/수정/실행/삭제 작업은 제한되는 임원용 조회 권한 사용자
...
> *executive/user는 자신의 정보만 수정 가능
> *executive/user가 비밀번호를 변경하려면 현재 비밀번호(current_password) 입력 필수. admin은 현재 비밀번호 없이 변경 가능
...
### **executive 사용자 제한사항**
- 테스트 케이스, 자동화 테스트, 성능 테스트, 폴더 등 주요 업무 데이터 생성/수정 불가
- 테스트 실행 및 업로드 불가
- admin 전용 시스템 설정 변경 불가
- 프로젝트 생성/수정/삭제 불가
- 사용자 생성/삭제 및 타 사용자 권한 변경 불가
...
4. 역할 선택 (admin, executive, user, guest)
...
@user_required       # admin, user 접근. executive는 GET/HEAD 조회만 접근
